...
- **إدارة الأبناء (Children Manager):** إمكانية إضافة حتى 5 أبناء، تحديد هدف النجوم الأسبوعي، مدة ظهور الابن على شاشة العرض (بالثواني)، ورفع صورة شخصية.
- **تصفير مسار الإنجاز:** إمكانية إعادة ضبط مسار الإنجاز ليبدأ الأرنب من نقطة البداية دون حذف النجوم أو السجلات السابقة من قاعدة البيانات.
- **إدارة الإنجازات (Achievements Manager):** إضافة مهام وإنجازات يومية مع صور تعبيرية، وتخصيص الإنجازات لأبناء محددين.
- **التقييم اليومي (Daily Evaluation):** واجهة مريحة وسريعة لتقييم كل ابن يومياً (نجمة أو إكس) كحد أقصى 5 تقييمات للإنجاز الواحد في اليوم. وتتضمن الواجهة عرض حي لـ "مسار الإنجاز" وتفاعلات مباشرة.
- **إدارة شاشات العرض (Kiosk Manager):** ربط أجهزة العرض (مثل أجهزة الآيباد، شاشات التلفاز) بسهولة عبر رمز PIN أو QR Code، مع إمكانية إلغاء ربط الشاشة من لوحة التحكم عن بُعد.
...
- **مسار الإنجاز البصري:** يحتوي مسار الإنجاز على شخصية كرتونية (أرنب على سكوتر) تتحرك بمرونة نحو "الميدالية" بناءً على فرق النجوم المطروحة من الإكسات.
...
- **مؤثرات بصرية وصوتية متجاوبة:** إطلاق تأثيرات "Confetti" وأصوات تشجيعية عند النجاح، وأصوات تنبيهية عند الإخفاق أو حذف التقييم.
- **تصميم متجاوب بالكامل:** يعتمد التصميم على وحدات (`clamp` و `container queries`) ليتجاوب تماماً مع جميع الشاشات (تلفاز، جهاز لوحي، أو جوال).
- **نظام أمني للشاشات:** تعمل الشاشات عبر نظام جلسات `Sessions` لا يتطلب تسجيل الدخول بكلمة المرور في التلفاز، بل عبر ربط آمن وسريع يدوم حتى يقوم الأب بإلغائه.
